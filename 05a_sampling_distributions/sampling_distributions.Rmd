...
The name of the data frame is `global_monitor` and the name of the variable that contains responses to the question *"Do you believe that the work scientists do benefit people like you?"* is `scientist_work`. The data is [here](./more/global_monitor.csv).
...
In this lab, because you have access to the population, you could build up the sampling distribution for the sample proportion by repeating the above steps many times. By repeating this process 15,000 times, we generated 15,000 samples of size 50 from the population, calculate the proportion of responses in each sample, and store the results, and saved these results in the file [here](more/sample_props50.csv). Load this dataset to get a sense of what it tells us.
