...
The name of the data frame is `global_monitor` and the name of the variable that contains responses to the question *"Do you believe that the work scientists do benefit people like you?"* is `scientist_work`. The data is [here](global_monitor.csv). 

We can quickly visualize the distribution of these responses using a bar plot. You can find this in the `Descriptives` menu, by clicking `Distribution plots` in under `Basic plots`.
...
If you are interested in estimating the proportion of people who don't think the work scientists do benefits them, you can use the `distrACTION` tool to survey the population.
...
Here, we use Jamovi to take 15,000 different samples of size 50 from the population, calculate the proportion of responses in each sample, and store the results.

Go back to the section of the Binomial menu where we generated the data. Create a new variable called `samples15000` and change the `Number of samples` to 15000 (but leave the other values as they were). Generate this variable, and go back to the dataset to see what has been created.

3. What does each value in the `samples15000` variable represent?

Next we will convert these counts to proportions to better compare the values. Create a new variable called `prop15000` given by the formula `samples15000 / 50`.

We can now visualize the distribution of these proportions with a histogram. Create a histogram of the newly created `prop15000` variable.

1.  How many elements are there in `prop15000`? Describe the sampling distribution, and be sure to specifically note its center. Make sure to include a plot of the distribution in your answer.
...
1.  Take a sample of size 15 from the population and calculate the proportion of people in this sample who think the work scientists do enhances their lives.
