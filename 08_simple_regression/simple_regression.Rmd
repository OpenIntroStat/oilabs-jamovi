...
The data we're working with is [here](more/hfi.csv), and it's called `hfi`, short for Human Freedom Index. Information about the variables can be seen [here](https://www.openintro.org/data/index.php?data=hfi).
...
We should also verify this condition with a plot of the residuals vs. fitted (predicted) values. Go back to the Linear Regression analysis, and open the `Model Fit` submenu. Check the `Residual plots` box and look at the plots that are created. We will focus on the first one, Residuals vs Fitted.
