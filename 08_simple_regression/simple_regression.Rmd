...
The Human Freedom Index is a report that attempts to summarize the idea of "freedom" using different variables for many countries around the globe.
...
In this lab, you'll be analyzing data from the Human Freedom Index reports.
...
The dataset spans a lot of years, but we are only interested in data from year 2016.  Filter the data `hfi` data frame for year 2016.

2. What type of plot would you use to display the relationship between the personal freedom score, `pf_score`, and `pf_expression_control`? Plot this relationship using the variable `pf_expression_control` as the predictor.

3. Does the relationship look linear? If you knew a country's `pf_expression_control` (political pressures and controls on media content) as a score out of 10, would you be comfortable using a linear model to predict the personal freedom score?
...
4.  Looking at your plot from the previous exercise, describe the relationship between these two variables. Make sure to discuss the direction and strength of the relationship, the value of the correlation coefficient, as well as any unusual observations.
...
You can use jamovi to fit the linear model (a.k.a. regression line). Click `Regression`, then `Linear Regression`. For the dependent variable, select `pf_score` and for covariates, select `pf_expression_control`.
...
Look at a scatter plot for these variables, and set the `Regression Line` option is set to `Linear`.  This will overlay the least-squares regression line over teh scatterplot.
...
6.  If someone saw the least squares regression line and not the actual data, how would they predict a country's personal freedom school for one with a rating of 3 for `pf_expression_control`?
...
8.  Based on the Q-Q plot, does the nearly normal residuals condition appear to be violated? Why or why not?
...
10. Choose another variable that you think would strongly correlate with `pf_score`. Produce a scatterplot of the two variables and fit a linear model. At a glance, does there seem to be a linear relationship?

11. How does this relationship compare to the relationship between `pf_score` and `pf_expression_control`? Use the $R^2$ values from the two model summaries to compare.  Does your independent variable seem to predict `pf_score` better?  Why or why not?

12.  Check the model diagnostics using appropriate visualizations and evaluate if the model conditions have been met.

13.  Pick another pair of variables of interest and visualize the relationship between them.  Do you find the relationship surprising or is it what you expected.  Discuss why you were interested in these variables and why you were/were not surprised by the relationship you observed.
